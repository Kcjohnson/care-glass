...
rule consensusvcf:
    input:
        vcfs = expand("results/mutect2/dropgt/{case_barcode}.filtered.normalized.sorted.dropgt.vcf.gz", case_barcode = manifest.getSelectedCasesLx()),
        tbis = expand("results/mutect2/dropgt/{case_barcode}.filtered.normalized.sorted.dropgt.vcf.gz.tbi", case_barcode = manifest.getSelectedCasesLx())
    output:
