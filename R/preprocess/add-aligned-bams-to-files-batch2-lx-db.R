...
lx_existing_aligned_wgs <- files %>% 
  filter(grepl("GLSS-LX", aliquot_barcode), grepl("aligned BAM", file_format))
...
realn_size <- read.delim("/projects/verhaak-lab/USERS/johnsk/glass4/results/db_uploads/GLSS_LX_batch2_realigned_filesize_filterme.txt", header=FALSE)
...
## Filter out any aliquots already in the database:
realn_size_filtered <- realn_size %>% 
  filter(!aliquot_barcode%in%lx_existing_aligned_wgs$aliquot_barcode)

# The GLSS-LX cohort
## cat *.md5sum > /projects/verhaak-lab/USERS/johnsk/glass4/results/db_uploads/glss_lx_batch2_realigned_md5.txt
realn_md5_lx <- read.delim("/projects/verhaak-lab/USERS/johnsk/glass4/results/db_uploads/glss_lx_batch2_realigned_md5.txt", sep=" ", header=FALSE)
...
lx_realn_files_new <- realn_size_filtered %>% 
...
## Upload these data to the glass4 database. Successfully uploaded.
dbWriteTable(con, Id(schema="analysis", table="files"), lx_realn_files_new, append=T)
