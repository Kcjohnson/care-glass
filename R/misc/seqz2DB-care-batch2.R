#################################
# Push sequenza seg + params into db
# Updated: 2022.07.06
...
## Read in existing Sequenza data
...
## Limit to new cohorts (CARE-TK-WXS and CARE-TO-WXS)
df_new <- df_new[grep("CARE-TK|CARE-TO", df_new[,"pair_barcode"]),]
...
#cd results/sequenza
#echo -e "pair_barcode\tcellularity\tploidy\tslpp" > care_batch2_seqz_params.txt

#for i in seqzR/CARE-T[OK]*/*_alternative_solutions.txt
#do 
#  V1=$(echo $i | cut -c 7-35)
#  V2=$(head -2 "$i" | tail -1)
#  echo -e "$V1\t$V2" >> care_batch2_seqz_params.txt
# done 

# Add parameters to db
dat <- read.delim("results/sequenza/care_batch2_seqz_params.txt")

old <- dbReadTable(con, Id(schema="variants",table="seqz_params"))
old_pairs <- unique(old[,"pair_barcode"])

## Here we explicitly added new data so we don't need to remove the old data before appending
any(dat[,"pair_barcode"] %in% old_pairs)
dat_new <- dat

## If we were interested in keeping only the new data
# dat_new <- dat[-which(dat[,"pair_barcode"] %in% old_pairs),]

# Limit to new cohorts (CARE-TK and CARE-TO)
# dat_new <- dat_new[grep("CARE-TK|CARE-TO", dat_new[,"pair_barcode"]),]
