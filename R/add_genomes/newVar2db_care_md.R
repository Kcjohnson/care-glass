...
dat <- read.delim("/projects/verhaak-lab/USERS/johnsk/glass4/results/mutect2/consensusvcf/CARE-MD/consensus.normalized.sorted.funcotated.tsv", colClasses = c("character","character",NULL,"character",rep(NULL,14)),header=FALSE)
...
myoutf <- "/projects/verhaak-lab/USERS/johnsk/glass4/results/mutect2/consensusvcf/consensus.normalized.sorted.funcotated.CARE-MD.tsv"
write.table(new_dat,myoutf,sep="\t",quote=F,row.names=F)

new_dat <- read.delim("/projects/verhaak-lab/USERS/johnsk/glass4/results/mutect2/consensusvcf/consensus.normalized.sorted.funcotated.CARE-MD.tsv",stringsAsFactor=FALSE)
