...
if(exists("snakemake")) {
  ...
  files = list.files("results/align/wgsmetrics", recursive = T, pattern = "WgsMetrics.txt", full.names = T) 
}

outfn <- "results/align/care_batch2_wgsmetrics.merged.tsv"
files <- files[grep("CARE-TK|CARE-TO", files)]
# Remove CARE-TK-TK06 and revisit later since it does not have a matched normal
files <- files[-grep("CARE-TK-TK06",files)]
...
## Combine all the samples from the CARE cohort.
care_cov = data.table::rbindlist(cov_dat)

# Cumulatively add the number of bases at each level:
care_samples_cumulative_cov = care_cov %>% 
  group_by(sample_id) %>% 
...
write.table(care_samples_cumulative_cov, file = outfn, quote = F, sep = "\t", row.names = FALSE, col.names = FALSE)

## Write the coverage file to the database.
dbWriteTable(con, Id(schema="analysis", table="coverage"), care_samples_cumulative_cov, append=TRUE)
